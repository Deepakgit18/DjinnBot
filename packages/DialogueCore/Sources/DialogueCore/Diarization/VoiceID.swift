...
public final class VoiceID {
    ...
    nonisolated(unsafe) public static let shared = VoiceID()
    public weak var delegate: VoiceIDDelegate?

    /// Fires after any mutation to enrolled voices (enroll, enhance, remove, setColor).
    public let voicesDidChange = PassthroughSubject<Void, Never>()
    ...
    public func prepare() async throws {
        ...
        if #available(macOS 26.0, iOS 26.0, *),
           let preloaded = await MainActor.run(body: { ModelPreloader.shared.diarizerModels }) {
            models = preloaded
        } else {
...
private extension Float {
    ...
    public func nonZeroOrDefault(_ fallback: Float) -> Float {
