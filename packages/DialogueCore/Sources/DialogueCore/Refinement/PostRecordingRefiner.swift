...
@MainActor
public final class RefinementProgress: ObservableObject {
    public static let shared = RefinementProgress()
...
public final class PostRecordingRefiner {
    ...
    public func refine(
        ...
        let progress = await RefinementProgress.shared
    ...
    private func transcribeFile(url: URL) async throws -> [WordTiming] {
        ...
        async let wordsFuture = Self.collectWords(from: transcriber)
    ...
    private static func collectWords(from transcriber: SpeechTranscriber) async throws -> [WordTiming] {
    ...
    public func convertWAVsToOpus(in meetingFolder: URL) async {
        let fm = FileManager.default
...
public enum RefinementError: Error, LocalizedError {
