...
@MainActor
public final class MeetingPlayer: NSObject, ObservableObject {
    ...
    public func playSegment(url: URL, from startTime: TimeInterval, to endTime: TimeInterval) {
    ...
    public func playFrom(url: URL, time startTime: TimeInterval) {
        segmentEndTime = nil
        if loadedURL == url && (isPlaying || isPaused) {
...
extension MeetingPlayer: AudioPlayer.Delegate {
    nonisolated public func audioPlayer(_ audioPlayer: AudioPlayer, playbackStateChanged playbackState: AudioPlayer.PlaybackState) {
        Task { @MainActor in
