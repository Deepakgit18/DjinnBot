...
@MainActor
final class MeetingIngestService: ObservableObject {
    ...
    @Published var hasSummary: Bool = false

    /// Whether the meeting has already been ingested (persisted on disk).
    @Published var isIngested: Bool = false
    ...
    func loadState(for meeting: SavedMeeting) {
    ...
    func ingestAndSummarize(meeting: SavedMeeting, entries: [TranscriptEntry]) {
    ...
    static func loadSummaryMarkdown(for meeting: SavedMeeting) -> String? {
